...
namespace AlarmClock.AlarmClock.Code.Managers
{
    public class TimeManager
    {
        ...
        private AccessTools.FieldRef<object, double> _universalTimeField = AccessTools.FieldRefAccess<double>(typeof(TimeWarp), "_autoWarpTargetUniversalTime");
        private void HandleTimeStop(UniverseModel um, TimeWarp tw, double timeDelta)
        {
            ...
            if (tw.IsAutoWarpEngaged && _universalTimeField(tw) > timeAsSeconds && secondsToTarget - timeDelta <= 10)
            {
                tw.CancelAutoWarp();
            }
